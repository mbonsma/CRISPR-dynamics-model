...
def e_effective_shifted(e, nbi_interp, nvj_interp, max_shift = 1000, direction = 'past'):
    """
...
def effective_e(nbi, nvi, all_phages, pv_type, e, theta, distance_matrix = None, distance_matrix_theta = None):
    """
    ...
    distance_matrix: can optionally provide the pre-calculated distance matrix
        based on all_phages
    """
    ...
    if pv_type == 'binary':
        ...
        if np.all(distance_matrix == None): # make distance matrix
        ...
        numerator_list = []
        denominator = (np.sum(nvi, axis = 1)) * np.array(np.sum(nbi, axis = 1))
        max_m = nbi.shape[1]
        
        # use sparse matrices
        nbi = sparse.coo_matrix(nbi)
        nbi = sparse.csr_matrix(nbi)
        
        nvi = sparse.coo_matrix(nvi)
        nvi = sparse.csr_matrix(nvi)
        
        prod = sparse.kron(nbi, nvi, format = 'csr')
        
        if pv_type == 'exponential':
            pv_matrix_sparse = sparse.coo_matrix((1 -  e * np.exp(- distance_matrix)))
            pv_matrix_sparse = sparse.csr_matrix(pv_matrix_sparse)
            
            for i in range(nbi.shape[0]):
                numerator = np.sum((prod[(nbi.shape[0])*i+i].reshape(max_m,max_m)).multiply(pv_matrix_sparse)) #need to use .multiply
                numerator_list.append(numerator)                
            e_effective_list += list(1 - np.array(numerator_list)/denominator)
        elif pv_type == 'exponential_025':
            pv_matrix_sparse = sparse.coo_matrix((1 -  e * np.exp(- 0.25* distance_matrix)))
            pv_matrix_sparse = sparse.csr_matrix(pv_matrix_sparse)
            for i in range(nbi.shape[0]):
                numerator = np.sum((prod[(nbi.shape[0])*i+i].reshape(max_m,max_m)).multiply(pv_matrix_sparse)) #need to use .multiply
                numerator_list.append(numerator)                
            ...
            if np.all(distance_matrix_theta == None): # create the theta pv version
            ...
            pv_matrix_sparse = sparse.coo_matrix(distance_matrix_theta)
            pv_matrix_sparse = sparse.csr_matrix(pv_matrix_sparse)
            for i in range(nbi.shape[0]): # iterate over time
                numerator = np.sum((prod[(nbi.shape[0])*i+i].reshape(max_m,max_m)).multiply(pv_matrix_sparse)) #need to use .multiply
                numerator_list.append(numerator)      
            e_effective_list += list(1 - np.array(numerator_list)/denominator)
            
    else:
        print("please enter a valid pv_type: 'binary', 'exponential', 'exponential_025', or 'theta_function'")
        e_effective_list = None
