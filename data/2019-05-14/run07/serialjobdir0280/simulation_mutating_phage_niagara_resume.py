...
def update_phages(new_phages, new_parents, all_phages, parent_list, phage_list,
                  mutation_times, x, t, v, m, numSpecies, numReactions):
    # add new phages
    #phage_count is j
    for count, phage in enumerate(new_phages):
        if phage not in all_phages:
            all_phages.append(phage)
            parent_list.append([all_phages.index(phage_list[new_parents[count]])])
            mutation_times.append([t])
        else:
            ind2 = all_phages.index(phage)
            parent_list[ind2].append(all_phages.index(phage_list[new_parents[count]]))
    ...
    return x, m, all_phages, phage_list, parent_list, mutation_times, numSpecies, numReactions, v
...
def tau_iteration(check, a, tau, x, t, m, v, num_mutations, phage_list, parent_list, mutation_times, all_phages, numSpecies, numReactions):
    ...
    # add new phages
    (x, m, all_phages, phage_list, parent_list, mutation_times, 
     numSpecies, numReactions, v) = update_phages(all_new_phages, 
                                all_new_parents, all_phages, parent_list, 
                                phage_list, mutation_times, x, t, v, m, numSpecies, 
                                numReactions)
                        
            
    return check, x, m, v, num_mutations, phage_list, parent_list, mutation_times, all_phages, numSpecies, numReactions
    
def gillespie_iteration(a, a0, x, t, dt, m, v, num_mutations, phage_list, 
                parent_list, mutation_times, all_phages, numSpecies, numReactions):
    ...
    if (k >= (6*m + 4) and k < (7*m + 4)) or (k >= (8+m)*m + 4): # these are the terms where v wins
        ...
        # add new phages
        new_parents = [j]*len(new_phages)
        (x_new, m, all_phages, phage_list, parent_list, mutation_times, 
         numSpecies, numReactions, v) = update_phages(new_phages, 
                            new_parents, all_phages, parent_list, 
                            phage_list, mutation_times, x_new, t, v, m, 
                            numSpecies, numReactions)
            
    else: # do the normal update
        x_new = x + v[:,k]
    
    return (x_new, m, v, num_mutations, phage_list, parent_list, mutation_times, 
            all_phages, numSpecies, numReactions)
...
def remove_zeros(x, m, v, phage_list, numSpecies, numReactions):
    """
    ...
    return x, m, v, phage_list, numSpecies, numReactions

def run_simulation_tau(f, c0, g, B, R, eta, pv, alpha, e, L, mu, max_save, epsilon, gen_max, timestamp):
    ...
    while t*g*c0 <= gen_max: # stop once desired generations are reached    
        ...
        if tau < 10/a0: # do 100 gillespie iterations
            ...
            for counter in range(200):
                ...
                (x, m, v, num_mutations, phage_list, parent_list, mutation_times, 
                 all_phages, numSpecies, numReactions) = gillespie_iteration(a, a0, x, 
                                t, dt, m, v, num_mutations, phage_list, parent_list, 
                                mutation_times, all_phages, numSpecies, numReactions)
                
                if np.any(x < 0): # don't save this population step
            ...
            (x, m, v, phage_list, numSpecies, numReactions) = remove_zeros(x, m, 
                            v, phage_list, numSpecies, numReactions)
            ...
            check = 1
            while check == 1:
                (check, x_new, m_new, v_new, num_mutations_new, phage_list_new, 
                 parent_list_new, mutation_times_new, all_phages_new, 
                 numSpecies_new, numReactions_new) = tau_iteration(check, 
                    a, tau, x, t, m, v, num_mutations, phage_list, parent_list, 
                    mutation_times, all_phages, numSpecies, numReactions)
                n_increment = 1
                
                # if the tau-leaping step was successful, assign values to main variables
                if check == 0: 
                    x = x_new
                    m = m_new
                    v = v_new
                    num_mutations = num_mutations_new
                    phage_list = phage_list_new
                    parent_list = parent_list_new
                    mutation_times = mutation_times_new
                    all_phages = all_phages_new
                    numSpecies = numSpecies_new
                    numReactions = numReactions_new
                    
                # if a negative population size was reached, try again with a smaller tau
                if check == 1: 
                    tau = tau/2 
                    n_increment = 1
                
                    # if we're still in the while loop and tau is smaller than a 
                    # gillespie timestep, do Gillespie instead
                    if tau < 1/a0: # do gillespie cycles
                        #print("Doing Gillespie for tau < 1/a0: tau = %s, a0 = %s" %(tau, a0))
                        check = 0 # bump out of tau loop
                        tau = 0 # increment t with Gillespie instead
                        n_increment = 0 # increment n with Gillespie instead
                        for counter in range(200):
                            a = total_rates_phage(numReactions, x, m, B, g, e, eta, pv, alpha, F, r, c0, mu, L)
                            a0 = np.sum(a)
                            n += 1 #increment n
                            ...
                            if np.any(x < 0): # don't save this population step
                                print("negative population from Gillespie (after tau)")
                                continue
                            else:                
                                t += dt
...
if __name__ == "__main__":
    ...
    num_mutations, t, x, phage_list = run_simulation_tau(f, c0, g, B, R, eta, pv, alpha, e, L, mu, max_save, epsilon, gen_max, timestamp)
