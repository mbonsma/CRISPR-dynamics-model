...
def total_rates_phage(numReactions, x, phage_list, m, B, g, e, eta, pv, alpha, F, r, c0, mu, L):
    """ 
    ...
    for i in range(m):
        ...
        for j in range(m):
            a[(7+i)*m+4+j] = alpha*(1-PV(i, j, phage_list, pv, e))*x[1+i]*x[m+1+j] # bi vs vj, bi wins
            a[(8+m+i)*m+4+j] = alpha*PV(i, j, phage_list, pv, e)*x[1+i]*x[m+1+j] # bi vs vj, vj wins
    
    return a
    
def PV(i,j, phage_list, pv, e):
    """
    Probability of success for phage j against bacterium i
    
    In this version, pv(i,j) falls off exponentially as the phage gets more
...
def run_simulation_tau(m_init, f, c0, g, B, R, eta, pv, alpha, e, L, mu, max_save, epsilon, gen_max):
    ...
    while t*g*c0 <= gen_max: # stop once desired generations are reached    
        ...
        a = total_rates_phage(numReactions, x, phage_list, m, B, g, e, eta, pv, alpha, F, r, c0, mu, L)
